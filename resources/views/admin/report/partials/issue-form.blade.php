<section class="space-y-6">
    <x-secondary-button x-data="" 
        x-on:click.prevent="$dispatch('open-modal', 'confirm-user-warning')">{{ __('Issue Warning') }}</x-secondary-button>

    <x-modal name="confirm-user-warning" :show="$errors->userWarning->isNotEmpty()" focusable>
        <form method="post" action="" class="p-6 bg-white rounded-lg shadow-md">
            ...
            <p class="mt-1 text-sm text-gray-600">
                {{ __('Please provide details for issuing a warning to this user.') }}
            </p>
