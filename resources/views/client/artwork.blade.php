<x-app-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6 min-h-screen">
        ...
        <form method="GET" action="{{ route('client.artwork') }}" class="mb-4">
            <!-- Top Row: Search, Sort, and Clear Button -->
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                <!-- Search -->
                <div class="sm:col-span-2">
                    <input type="text" name="search" placeholder="Search by title..."
                        class="w-full px-4 py-2 border border-gray-300 rounded-md text-black focus:border-gray-400 focus:ring-2 focus:ring-gray-300 transition"
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button type="submit" name="sort" value="{{ request('sort') == 'random' ? '' : 'random' }}"
                        class="flex-1 px-4 py-2 border border-gray-300 rounded-full transition
                ...
                {{ request('sort') == 'latest' ? 'bg-black text-white' : 'text-black hover:bg-gray-200' }}">
                    ...
                    <!-- Clear Button -->
                    <button type="button" onclick="window.location='{{ route('client.artwork') }}'"
                        class="px-4 py-2 border rounded-full bg-red-500 text-white hover:bg-red-600 transition">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                ...
                <div>
                    <select name="status" onchange="this.form.submit()"
                        class="w-full px-4 py-2 border border-gray-300 rounded-md text-black focus:border-gray-400 focus:ring-2 focus:ring-gray-300 appearance-none transition">
                ...
                <div class="flex items-center gap-2">
                    <input type="number" name="min_price" placeholder="Min Price"
                        class="w-full px-2 py-2 border border-gray-300 rounded-md text-black focus:border-gray-400 focus:ring-2 focus:ring-gray-300 transition"
                ...
                <div>
                    <button type="submit" name="discounted" value="{{ request('discounted') ? '' : '1' }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-full transition 
            ...
            <div class="relative w-full h-[250px] rounded-lg overflow-hidden shadow-lg">
                <a href="{{ route('artwork.show', $artwork) }}">
                    ...
                    <div class="absolute top-4 left-4 text-white">
                        <span
                            class="text-sm {{ $artwork->status == 'sale' ? 'bg-green-500' : 'bg-red-500' }} px-2 py-1 rounded-full">
