<x-app-layout>
    ...
    <div class="max-w-5xl mt-8 mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div class="relative">
            <div class="absolute top-0 left-0 w-full h-1 bg-gray-200">
                <div id="progress-bar" class="progress-animation h-full
                    {{ $commission->status == 'completed' ? 'bg-green-500 w-full' : 
                       ($commission->status == 'done' ? 'bg-blue-500 w-3/4' : 
                       ($commission->status == 'wip' ? 'bg-yellow-500 w-1/2' : 
                       ($commission->status == 'ready' ? 'bg-blue-500 w-1/4' : 
                       ($commission->status == 'cancelled' ? 'bg-red-500 w-full' : 
                       ($commission->status == 'hold' ? 'bg-orange-500 w-1/2' : 
                       ($commission->status == 'returned' ? 'bg-purple-500 w-full' : 'bg-gray-500 w-0')))))) }}">
                </div>
            </div>

            <div class="p-6 pt-8 bg-gradient-to-r 
                {{ $commission->status == 'completed' ? 'from-green-50 to-green-100' : 
                   ($commission->status == 'done' ? 'from-blue-50 to-blue-100' : 
                   ($commission->status == 'wip' ? 'from-yellow-50 to-yellow-100' : 
                   ($commission->status == 'ready' ? 'from-blue-50 to-blue-100' : 
                   ($commission->status == 'cancelled' ? 'from-red-50 to-red-100' : 
                   ($commission->status == 'hold' ? 'from-orange-50 to-orange-100' : 
                   ($commission->status == 'returned' ? 'from-purple-50 to-purple-100' : 'from-gray-50 to-gray-100')))))) }}">
                ...
                <p class="text-sm text-gray-600">
                    ...
                    Expected delivery: {{ \Carbon\Carbon::parse($commission->delivery->expected_delivery)->subDays(5)->format('j') }} -
        ...
        <div class="py-4 px-6 border-b border-gray-100">
            ...
            {{ $commission->status == 'completed' ? 'bg-green-100 text-green-800' : 
               ($commission->status == 'done' ? 'bg-blue-100 text-blue-800' : 
               ...
               ($commission->status == 'hold' ? 'bg-orange-100 text-orange-800' : 
               ($commission->status == 'returned' ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-800')))))) }}">
            ...
            <div class="flex items-center justify-between relative mt-10">
                ...
                <div class="absolute top-4 left-4 right-4 h-2 bg-gray-200 z-0 rounded-full overflow-hidden">
                    <div class="h-2 rounded-full progress-line"
                        style="--progress-width: {{ $commission->status == 'completed' ? '100%' : 
                               ($commission->status == 'done' ? '65%' : 
                               ($commission->status == 'wip' ? '33%' : 
                               ($commission->status == 'ready' ? '10%' : '0%'))) }}">
                    </div>
                </div>
                ...
                {{ $commission->status == 'ready' || $commission->status == 'wip' || $commission->status == 'done' || $commission->status == 'completed' ? 
                   'bg-blue-500 shadow-lg shadow-blue-200' : 'bg-gray-300' }} icon-animation hover:scale-105 transition-transform duration-300">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                            viewBox="0 0 20 20" fill="none" stroke="#ffffff" stroke-width="2"
                            stroke-linecap="round" stroke-linejoin="round">
                            <path d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" />
                        </svg>
                    </div>
                    <span class="text-sm font-medium mt-2 text-animation
                {{ $commission->status == 'ready' || $commission->status == 'wip' || $commission->status == 'done' || $commission->status == 'completed' ? 
                   'text-blue-500' : 'text-gray-400' }}">Ready</span>
                </div>
                ...
                {{ $commission->status == 'wip' || $commission->status == 'done' || $commission->status == 'completed' ? 
                   ...
                   ($commission->status == 'ready' ? 'bg-white border-2 border-blue-500 animate-pulse' : 'bg-gray-300') }} icon-animation-delay-1 hover:scale-105 transition-transform duration-300">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                            viewBox="0 0 20 20" fill="none" stroke="{{ $commission->status == 'wip' || $commission->status == 'done' || $commission->status == 'completed' ? '#ffffff' : ($commission->status == 'ready' ? '#3b82f6' : '#9ca3af') }}" stroke-width="2"
                            stroke-linecap="round" stroke-linejoin="round">
                            <path d="M10 2a1 1 0 011 1v1a1 1 0 11-2 0V3a1 1 0 011-1zm4 8a4 4 0 11-8 0 4 4 0 018 0zm-.464 4.95l.707.707a1 1 0 001.414-1.414l-.707-.707a1 1 0 00-1.414 1.414zm2.12-10.607a1 1 0 010 1.414l-.706.707a1 1 0 11-1.414-1.414l.707-.707a1 1 0 011.414 0zM17 11a1 1 0 100-2h-1a1 1 0 100 2h1zm-7 4a1 1 0 011 1v1a1 1 0 11-2 0v-1a1 1 0 011-1zM5.05 6.464A1 1 0 106.465 5.05l-.708-.707a1 1 0 00-1.414 1.414l.707.707zm1.414 8.486l-.707.707a1 1 0 01-1.414-1.414l.707-.707a1 1 0 011.414 1.414zM4 11a1 1 0 100-2H3a1 1 0 000 2h1z" />
                        </svg>
                    </div>
                    <span class="text-sm font-medium mt-2 text-animation-delay-1
                {{ $commission->status == 'wip' || $commission->status == 'done' || $commission->status == 'completed' ? 
                   'text-blue-500' : 
                   ($commission->status == 'ready' ? 'text-blue-500' : 'text-gray-400') }}">In Progress</span>
                </div>
                ...
                {{ $commission->status == 'done' || $commission->status == 'completed' ? 
                   'bg-blue-500 shadow-lg shadow-blue-200' : 
                   ($commission->status == 'wip' ? 'bg-white border-2 border-blue-500 animate-pulse' : 'bg-gray-300') }} icon-animation-delay-2 hover:scale-105 transition-transform duration-300">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                            viewBox="0 0 20 20" fill="none" stroke="{{ $commission->status == 'done' || $commission->status == 'completed' ? '#ffffff' : ($commission->status == 'wip' ? '#3b82f6' : '#9ca3af') }}" stroke-width="2"
                            stroke-linecap="round" stroke-linejoin="round">
                            <path d="M8 16.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM15 16.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0z" />
                            <path d="M3 4a1 1 0 00-1 1v10a1 1 0 001 1h1.05a2.5 2.5 0 014.9 0H10a1 1 0 001-1V5a1 1 0 00-1-1H3zM14 7a1 1 0 00-1 1v6.05A2.5 2.5 0 0115.95 16H17a1 1 0 001-1v-5a1 1 0 00-.293-.707l-2-2A1 1 0 0015 7h-1z" />
                        </svg>
                    </div>
                    <span class="text-sm font-medium mt-2 text-animation-delay-2
                {{ $commission->status == 'done' || $commission->status == 'completed' ? 
                   'text-blue-500' : 
                   ($commission->status == 'wip' ? 'text-blue-500' : 'text-gray-400') }}">Ready to Ship</span>
                </div>
                ...
                {{ $commission->status == 'completed' ? 
                   ...
                   ($commission->status == 'done' ? 'bg-white border-2 border-blue-500 animate-pulse' : 'bg-gray-300') }} icon-animation-delay-3 hover:scale-105 transition-transform duration-300">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                            viewBox="0 0 20 20" fill="none" stroke="{{ $commission->status == 'completed' ? '#ffffff' : ($commission->status == 'done' ? '#3b82f6' : '#9ca3af') }}" stroke-width="2"
                            stroke-linecap="round" stroke-linejoin="round">
                            <path d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" />
                        </svg>
                    </div>
                    <span class="text-sm font-medium mt-2 text-animation-delay-3
                {{ $commission->status == 'completed' ? 
                   'text-green-500' : 
                   ($commission->status == 'done' ? 'text-blue-500' : 'text-gray-400') }}">Completed</span>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-lg shadow-sm p-5 border border-gray-100">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex justify-between items-center border-b border-gray-100 pb-3">
                        ...
                        <span class="font-semibold text-red-500">₱{{ number_format($commission->request->total_price, 2) }}</span>
                    </div>
                    <div class="flex justify-between items-center border-b border-gray-100 pb-3">
                        <span class="text-gray-500">Dimension</span>
                        <span class="font-semibold">{{ $commission->request->width }} x {{ $commission->request->height }} {{ $commission->request->unit }}</span>
                    </div>
                    <div class="flex justify-between items-center border-b border-gray-100 pb-3">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between items-center border-b border-gray-100 pb-3">
                        ...
                        <span class="font-semibold px-2 py-1 rounded-full text-xs 
                            {{ $commission->delivery->status == 'pending' ? 'bg-yellow-100 text-yellow-800' : 
                              ($commission->delivery->status == 'in-transit' ? 'bg-blue-100 text-blue-800' : 
                              ($commission->delivery->status == 'delivered' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800')) }}">
                    ...
                    <div class="flex justify-between items-center border-b border-gray-100 pb-3">
                        ...
                        <span class="font-semibold text-right">{{ $commission->delivery->address->barangay }}, {{ $commission->delivery->address->street }}, House No. {{ $commission->delivery->address->house_number }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-gray-500">Expected Delivery</span>
                        <span class="font-semibold">{{ \Carbon\Carbon::parse($commission->delivery->expected_delivery)->subDays(5)->format('j') }} -
            ...
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    ...
                    <div class="mt-1 text-sm text-yellow-700">
                        <p>Your commission has been granted an extension of 7 days. Please note the updated delivery date.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="p-6 border-t border-gray-100">
            <div class="flex flex-col md:flex-row justify-between items-center">
                <div class="flex items-center mb-4 md:mb-0">
                    ...
                    <p class="text-gray-600">Having issues? <a href="mailto:torchtech2024@gmail.com" class="text-blue-500 hover:text-blue-700 transition-colors">Contact Support</a></p>
                </div>
                ...
                <div class="flex space-x-3">
                    ...
                    <form action="{{ route('client.commission.receive', $commission) }}" method="POST">
                        @csrf
                        <button type="submit" class="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg shadow-sm transition-colors">
                            Mark as Received
                        </button>
                    </form>
                ...
                <form action="{{ route('client.review.commission', $commission) }}" method="POST" class="p-4">
                    ...
                    <div class="grid gap-4 mb-4 grid-cols-2">
                        <div class="mb-4" x-data="{ rating: 0 }">
                            ...
                            <div class="flex items-center">
                                <template x-for="star in 5">
                                    <svg @click="rating = star" :class="rating >= star ? 'text-yellow-300' : 'text-gray-300'" class="w-8 h-8 cursor-pointer transition duration-200" fill="currentColor" viewBox="0 0 22 20" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div class="col-span-2">
                            <label for="comment" class="block mb-2 text-sm font-medium text-gray-900">Your Review</label>
                            <textarea name="comment" id="comment" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500" placeholder="Share your experience..."></textarea>
                        </div>
                    </div>
    ...
    <div id="return-refund-modal" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow-lg">
                <button type="button" class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center" data-modal-hide="return-refund-modal">
                    <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                    </svg>
                    <span class="sr-only">Close modal</span>
                ...
                <div class="p-6 text-center">
                    <svg class="mx-auto mb-4 w-12 h-12 text-yellow-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                    </svg>
                    <h3 class="mb-5 text-lg font-normal text-gray-800">Return/Refund Request</h3>
                    <p class="mb-4 text-sm text-gray-600">Are you sure you want to request a return/refund? Please note that a 10% service fee will be deducted from your refund amount.</p>

                    <form action="{{ route('client.return.commission', $commission) }}" method="POST">
                        @csrf
                        <div class="mb-4">
                            <label for="reason" class="block mb-2 text-sm font-medium text-gray-900 text-left">Reason for return/refund:</label>
                            <textarea id="reason" name="reason" rows="3" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500" placeholder="Please explain why you're requesting a refund..."></textarea>
                        </div>

                        <div class="flex justify-center space-x-4">
                            <button type="button" class="py-2 px-4 text-gray-500 bg-white hover:bg-gray-100 rounded-lg border border-gray-200 text-sm font-medium focus:ring-4 focus:outline-none focus:ring-gray-200" data-modal-hide="return-refund-modal">
                                Cancel
                            </button>
                            <button type="submit" class="py-2 px-4 text-white bg-yellow-600 hover:bg-yellow-700 focus:ring-4 focus:outline-none focus:ring-yellow-300 font-medium rounded-lg text-sm">
                                Submit Request
                            </button>
                        </div>
                    </form>
