                    ...
                    <div class="relative w-full h-[250px] rounded-lg overflow-hidden shadow-lg">
                        <a href="{{ route('client.request.show', $request) }}">
                            ...
                            <img src="{{ $thumbnail ? asset('storage/' . $thumbnail->path) : asset('images/default-image.jpg') }}"
                                alt="{{ $service->category->name ?? 'service' }}" class="w-full h-full object-cover">
                            ...
                            <div class="absolute bottom-4 right-4 text-white">
                                <p class="text-sm mt-1"><span class="font-bold">{{ ucfirst($request->status ?? 'Pending') }}</span></p>
                            </div>
                        </a>
                    </div>
                @endforeach
                    ...
                    <div class="relative w-full h-[250px] rounded-lg overflow-hidden shadow-lg">
                        <a href="{{ route('client.commission.show', $commission) }}">
                            ...
                            <div class="absolute top-4 left-4 right-4 flex justify-between items-center">
                                ...
                                <span class="text-xl font-semibold">₱{{ number_format($commission->request->total_price, 0, '.', ',') }}</span>
                            </div>
                            ...
                            <!-- Status -->
                            <div class="absolute bottom-4 right-4 text-white">
                                <p class="text-sm mt-1"><span class="font-bold">{{ ucfirst($commission->status ?? 'Pending') }}</span></p>
                            </div>
                        </a>
                    </div>
                @endforeach
