        ...
        <div class="flex flex-column sm:flex-row flex-wrap space-y-4 sm:space-y-0 items-center justify-between pb-4">
            ...
            <script>
                document.getElementById('table-search').addEventListener('input', function () {
                    ...
                    document.getElementById('no-results').style.display = hasVisibleRows ? 'none' : '';
                });

                document.querySelectorAll('input[name="status-radio"]').forEach(radio => {
                    radio.addEventListener('change', function () {
                        let filter = this.value.toLowerCase();
                        let rows = document.querySelectorAll('tbody tr');
                        let hasVisibleRows = false;
                        rows.forEach(row => {
                            let status = row.querySelector('td:nth-child(6)').textContent.toLowerCase();
                ...
                @foreach ($orders as $order)
                    <tr
                        ...
                        <th scope="row"
                            ...
                            {{ $order->items->first()->artwork->title }}
                        </th>
                        <td class="px-6 py-4">
                            {{ $order->client->user->name }}
                        </td>
                        <td class="px-6 py-4">
                            {{ $order->items->first()->artwork->category->name }}
                        </td>
                        <td class="px-6 py-4">
